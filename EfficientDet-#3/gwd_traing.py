...
marking = pd.read_csv(r'D:\Workspace\Python\GWD\data\train.csv')
...
df_folds.loc[:, 'bbox_count'] = 1# 这里重新创建一个列名字是bboxcount
df_folds = df_folds.groupby('image_id').count()#将id相同的合并到同一个队列之中
...
TRAIN_ROOT_PATH = r'D:\Workspace\Python\GWD\data\train'
...
class DatasetRetriever(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.test or random.random() > 0.66:
            image, boxes = self.load_image_and_boxes(index)
        elif self.test or 0.66>=random.random()>0.33:
