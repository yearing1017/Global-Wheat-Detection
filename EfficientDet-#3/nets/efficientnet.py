...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params):
        ...
        # 对于momentum 当barch size比较小的时候 选择0.9-0.99 batch size比较大的时候 选择0.6-0.8
        self._bn_eps = global_params.batch_norm_epsilon

        # 注意力机制的缩放比例 # 这里就是先缩小再扩大 引入注意力机制
        ...
        self._depthwise_conv = Conv2d(
            ...
            kernel_size=k, stride=s, bias=False)#这里的group是深度可以分离卷积 对于每一个group都进行卷积
...
#  综上 可以看出 efficientdet模型 是先经过了一个
# （1）1x1卷积扩张通道数扩张expand_ratio的倍数(处了第一层) 
# （2）深度可分离卷积 有的3x3 有的5x5
# （3） 注意力机制 先压缩通道再拉伸通道1x1卷积操作
# （4） 输出经过1x1卷积处理通道数
    def forward(self, inputs, drop_connect_rate=None):
        x = inputs
        # 这里两步完成上面的1，2步骤着实有点精辟
        if self._block_args.expand_ratio != 1:
        ...
        # 添加了注意力机制
        if self.has_se:
            # 这里进行了一个全局的池化 池化成为一个一维向量（X x 1）
            x_squeezed = F.adaptive_avg_pool2d(x, 1)
            x_squeezed = self._se_expand(
                self._swish(self._se_reduce(x_squeezed)))
            # 这里添加每一个feature map的重要程度
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None):
        ...
        for i in range(len(self._blocks_args)):
            ...
            # 剩下的几个重复的部分 只是需要将输入通道变为原来的输出通道再进行卷积即可
